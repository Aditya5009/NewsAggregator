...
public class SozcuParser implements NewsParser {
	...
	public List<News> parse(JSONObject jsonObject) {
		...
		return stream.map(item -> {
			...
			return News.builder().title(item.getString("title")).link(item.getString("link"))
					.content(item.getString("description"))
					.publishedDate(LocalDateTime.parse(item.getString("pubDate"), formatter)).publisher("Sozcu")
