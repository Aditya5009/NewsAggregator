...
public class NTVParse implements NewsParser {
	...
	public List<News> parse(JSONObject jsonObject) {
		...
		return stream.map(item -> {
			...
			return News.builder().title(item.getJSONObject("title").getString("content")).link(item.getString("id"))
					.content(htmlContent.replaceAll("<.*?>", "")).imageUrl(imageUrl)
					.publishedDate(LocalDateTime.parse(item.getString("published"), formatter)).publisher("NTV")
					.build();
